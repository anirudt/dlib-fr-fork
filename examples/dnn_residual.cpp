...
std::vector<std::vector<string>> load_objects_list (
    const string& subj_req
)
{
    std::ifstream jsonFile("/home/anirudt/Projects/AtDisp/data.json");
    ...
    for (auto& subj: pt) {
      string subject = subj.first;
      if (subject.compare(subj_req)) {
        continue;
      }
      for (auto& roll: subj.second) {
        std::vector<std::string> imgs;
        for (auto roll_list: as_vector<std::string>(subj.second, roll.first)) {
          imgs.push_back("/home/anirudt/Projects/AtDisp/polls/static/polls/EE1/"+roll_list);
        }
...
)
{
    ...
    for (size_t i = 0; i < num_people; ++i)
    {
        ...
        for (size_t j = 0; j < samples_per_id; ++j)
        {
            ...
            matrix<rgb_pixel> image(150, 150); 
            std::vector<unsigned long> id_vecs;
            id_vecs.assign(10, id);
            std::vector<matrix<rgb_pixel>> augmented;
            load_image(img, obj);
            resize_image(img, image);
            augmented = jitter_image(image, 10);
            images.insert(images.end(), augmented.begin(), augmented.end());
            labels.insert(labels.end(), id_vecs.begin(), id_vecs.end());
        }
    }
...
int main(int argc, char** argv)
{
    ...
    cout << "objs[0].size():" << objs[0].size() << endl;
